...
// Oluşturduğunuz Array in içerinde kaç adet 125 sayısı vardır?
    ...
    public static void main(String[] args) {
        ...
        System.out.println(rasgeleArrayKontrol(5000,132));
    }
    public static  int rasgeleArrayKontrol(int boyut,int kontrolEdilecekRakam){
        Random random=new Random();
        int [] yeniArray=new int[boyut];
        int count=0;
        for(int i=0; i<boyut; i++){
            yeniArray[i]=random.nextInt(20)+120;
            if(yeniArray[i]==kontrolEdilecekRakam){
                count++;
            }
        }
        return count;
